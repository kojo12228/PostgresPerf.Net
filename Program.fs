...
type CopyVsInsertUnnestComparison () =
        ...
        task {
            ...
            seq {
                for i in 1..this.Insertions do
                    ...
                    (DateTime.UnixEpoch.AddDays (Random.Shared.NextDouble () * 10000.0)).ToUniversalTime ()
            }
            |> data.AddRange
        }
    ...
    member this.InsertUnnest () =
        task {
            use command = dataSource.CreateCommand """
                ...
                """
            command.Parameters.Add (NpgsqlParameter<int []>("ids", data |> Seq.map (fun (i, _, _) -> i) |> Seq.toArray)) |> ignore
            command.Parameters.Add (NpgsqlParameter<string []>("names", data |> Seq.map (fun (_, n, _) -> n) |> Seq.toArray)) |> ignore
            command.Parameters.Add (NpgsqlParameter<DateTime []>("dates", data |> Seq.map (fun (_, _, d) -> d) |> Seq.toArray)) |> ignore
    ...
    member this.Copy () =
        task {
            ...
            let! importer = connection.BeginBinaryImportAsync """
...
[<EntryPoint>]
let main argv =
    let summary = BenchmarkDotNet.Running.BenchmarkRunner.Run<CopyVsInsertUnnestComparison> ()
    0
